...
def get_prompt(gen_type: int, session, style_id: uuid.UUID):
    if gen_type == GenerationType.HAIRCUT.value:
        ...
        prompt = f"""Возьми приложенное фото. Сохрани полностью лицо человека, черты лица, пол, возраст, выражение лица, текстуру кожи и одежду. Единственное, что нужно изменить — это цвет волос.
        Новый цвет волос (название): {color.name}.
        Точный цветовой код (HEX) для ориентира: {color.hex}.      
        Важные требования:   
        - Сохрани ТЕКУЩУЮ длину, стрижку и текстуру волос (пусть они останутся такими же, как на фото, меняется только пигмент).
        - Новый цвет должен выглядеть естественно: он должен быть равномерно распределен от корней до кончиков, с реалистичными бликами и тенями, которые соответствуют исходному освещению на фото.
        - Линия роста волос (край у лба и висков) должна остаться родной, без эффекта «парика».
        - Не меняй фон, позу, одежду, цвет глаз и цвет кожи. Результат должен выглядеть как то же самое фото, но с другим, натуральным цветом волос."""
    return prompt
