...
export default function MasterPriceListPage() {
  ...
  const [portfolio, setPortfolio] = useState<WorkFile[]>([]);
  ...
  useEffect(() => {
    const fetchAll = async () => {
      ...
      try {
        // 1. Прайс-лист
        const priceRes = await fetch(`${baseUrl}/users/price/?master_id=${masterId}`);
        if (!priceRes.ok) throw new Error("Ошибка загрузки прайс-листа");
        const priceData = await priceRes.json();
        if (priceData.status !== "success") throw new Error(priceData.status);
        const mappedCategories: Category[] = priceData.categories.map((cat: CategoryTemplate) => ({
        ...
        const infoRes = await fetch(`${baseUrl}/users/price/master_info?master_id=${masterId}`);
        if (!infoRes.ok) throw new Error("Ошибка загрузки информации о мастере");
        const infoData = await infoRes.json();
        if (infoData.status !== "success") throw new Error(infoData.status);
        setMasterInfo({
          name: infoData.name,
        ...
        // 3. Портфолио (если есть)
        if (infoData.portfolio) {
          const portRes = await fetch(`${baseUrl}/users/price/master_portfolio?master_id=${masterId}`);
          if (portRes.ok) {
            const portData = await portRes.json();
            if (portData.status === "success") {
              setPortfolio(portData.files || []);
            }
          }
        }
      ...
      <div className="max-w-sm mx-auto px-4 pb-10 relative">
        ...
        {masterInfo && (
          <section className="mt-6">
            <h2 className="text-[32px] tracking-[-1.6px] font-['Sofia_Sans'] text-black">Мастер</h2>
            <div className="h-px bg-black w-[210px] mb-4" />
            <div className="flex items-start gap-4">
              <img
                src={masterInfo.avatar || "https://placehold.co/80x80"}
                alt="avatar"
                className="w-20 h-20 rounded-full object-cover border border-black/10 flex-shrink-0"
              />
              <div className="flex-1 min-w-0">
                <p className="text-[18px] font-['Sofia_Sans'] text-black font-semibold break-words">
                  {masterInfo.name}
                </p>
                <p className="text-[14px] font-['Sofia_Sans'] text-black/70 break-words whitespace-pre-wrap mt-1">
                  {masterInfo.description || "О себе не заполнено"}
                </p>
                {masterInfo.portfolio && (
                  <Link
                    to={`/master-portfolio/${masterId}`}
                    state={{ portfolio }}
                    className="inline-block mt-3 bg-[#FFE9EF] rounded-[10px] py-1.5 px-4 shadow-sm text-[14px] tracking-[-0.7px] font-['Sofia_Sans'] text-black"
                    style={{
