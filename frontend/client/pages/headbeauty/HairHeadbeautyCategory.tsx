...
const femaleHairCategories = [
  { label: "Стрижки", image: haircutIcon },
  { label: "Окраска", image: coloringIcon },
  { label: "Завивка", image: permIcon },
];

const maleHairCategories = [
  { label: "Стрижки", image: haircutIcon },
  { label: "Барберинг", image: beardIcon },
  { label: "Окраска", image: coloringIcon },
  { label: "Завивка", image: permIcon },
];
...
export default function AIHairPage() {
  ...
  const categories = selectedGender === "male" ? maleHairCategories : femaleHairCategories;
        ...
        <div className="flex justify-center flex-nowrap gap-2 px-2 pb-4">
          {categories.map((cat, idx) => (
            <div
              ...
              onClick={() => {
                // Здесь будет переход на страницу записи к мастеру (заглушка)
                alert(`Выбрана категория: ${cat.label}`);
              }}
            >
