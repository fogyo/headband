...
export default function AIColorPage() {
  ...
  const [recommendedColors, setRecommendedColors] = useState<Color[]>([]);
  ...
  useEffect(() => {
    const fetchColors = async () => {
      ...
      try {
        const res = await fetch(`${baseUrl}/headbeauty/session/colors?session_id=${effectiveSessionId}`);
  ...
  const handleGetRecommendations = async () => {
    ...
    try {
      const res = await fetch(`${baseUrl}/headbeauty/session/color_recommends?session_id=${effectiveSessionId}`);
      ...
      const readyRes = await fetch(`${baseUrl}/headbeauty/session/ready_color_recommendations?session_id=${effectiveSessionId}`);
  ...
  const recommendedIds = new Set(safeRecommendedColors.map(c => c.id));
  const otherColors = safeAllColors.filter(c => !recommendedIds.has(c.id));
