...
const STATIC_CHAT_ID = 980609742; // TODO: заменить на реальный chat_id
...
// Загрузка файла через presigned URL
...
// Мок для редактирования (заглушка)
  ...
  const handleSave = async () => {
    ...
    try {
      if (type === "text") {
        ...
        const videoKey = await uploadFile(videoData.file);
        // Обложка пока не используется в API, но можно передать, если бэк поддержит
        ...
        const data = await res.json();
        if (data.status !== "success") throw new Error(data.status);
        toast.success(isEditing ? "Видео гайд обновлён" : "Видео гайд отправлен на согласование");
        navigate("/profile/guides");
      }
    } catch (err: any) {
      console.error(err);
                  ...
                  <div key={step.id} className="flex gap-3 items-stretch">
                    ...
                    <div className="flex-1 flex flex-col gap-3">
                      <div className="relative bg-[#FFE9EF] rounded-[10px] h-10 shadow" style={{ border: "0.5px solid rgba(0,0,0,0.00)", boxShadow: "57px 60px 23px 0 rgba(0,0,0,0.00), 36px 38px 21px 0 rgba(0,0,0,0.01), 20px 22px 18px 0 rgba(0,0,0,0.05), 9px 10px 13px 0 rgba(0,0,0,0.09), 2px 2px 7px 0 rgba(0,0,0,0.10)" }}>
                        <input type="text" placeholder="Название шага" value={step.title} onChange={e => updateStep(step.id, "title", e.target.value)} className="w-full h-full bg-transparent text-[16px] tracking-[-0.8px] font-['Sofia_Sans'] text-black outline-none text-center placeholder-black/50" />
                      </div>
                      <div className="relative bg-[#FFE9EF] rounded-[10px] h-36 shadow" style={{ border: "0.5px solid rgba(0,0,0,0.00)", boxShadow: "57px 60px 23px 0 rgba(0,0,0,0.00), 36px 38px 21px 0 rgba(0,0,0,0.01), 20px 22px 18px 0 rgba(0,0,0,0.05), 9px 10px 13px 0 rgba(0,0,0,0.09), 2px 2px 7px 0 rgba(0,0,0,0.10)" }}>
                        <textarea placeholder="Описание шага" value={step.description} onChange={e => updateStep(step.id, "description", e.target.value)} className="w-full h-full bg-transparent text-[16px] tracking-[-0.8px] font-['Sofia_Sans'] text-black outline-none resize-none p-3 placeholder-black/50" />
                      </div>
                    ...
                    {steps.length > 1 && <button onClick={() => removeStep(step.id)} className="w-6 h-6 flex items-center justify-center text-black/50"><X className="w-4 h-4" /></button>}
                  </div>
                ))}
              </div>
              <button onClick={addStep} className="mt-10 w-full bg-[#FFE9EF] rounded-[10px] h-10 shadow flex items-center justify-center" style={{ border: "0.5px solid rgba(0,0,0,0.00)", boxShadow: "57px 60px 23px 0 rgba(0,0,0,0.00), 36px 38px 21px 0 rgba(0,0,0,0.01), 20px 22px 18px 0 rgba(0,0,0,0.05), 9px 10px 13px 0 rgba(0,0,0,0.09), 2px 2px 7px 0 rgba(0,0,0,0.10)" }}><span className="text-[16px] tracking-[-0.8px] font-['Sofia_Sans'] text-black">Добавить еще шаг</span></button>
            </>
          )}
